...
def account_template(account):
    ...
    graql_insert_query += ", has opening-date " + str(account["opening-date"][:-3])
    graql_insert_query += ";"
    return graql_insert_query
...
def card_template(card):
    ...
    graql_insert_query += ", has expiry-date " + card["expiry-date"][:-3]
    graql_insert_query += ", has created-date " + card["created-date"][:-3]
    graql_insert_query += ";"
    return graql_insert_query
...
def contract_template(contract):
    ...
    graql_insert_query += "$contract has sign-date " + contract["sign-date"][:-3] + "; "
