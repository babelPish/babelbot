...
class GraphDatabase(KnowledgeBase):
    ...
    def _thing_to_dict(self, transaction, thing):
        """
        ...
        print(entity)
        for each in thing.as_remote(transaction).attributes():
        ...
        with GraknClient(uri=self.uri) as client:
            with client.session(keyspace=self.keyspace) as session:
                with session.transaction().read() as tx:
                    ...
                    #concepts = result_iter.map() #collect_concepts()
                    concepts = [ans.get("bank") for ans in result_iter]
                    print(concepts)
                    #return [c.value() for c in concepts]
                    entities = []
                    for c in concepts:
                        entities.append(self._thing_to_dict(tx, c))
                    return entities
